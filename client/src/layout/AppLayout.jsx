  ...
  return (
    <div className="relative" ref={ref}>
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className={`hl-nav-link ${open ? 'bg-slate-100 text-brand-navy' : ''}`}
      >
        Plus
        <IconChevronDown className={`h-3.5 w-3.5 transition-transform ${open ? 'rotate-180' : ''}`} />
      </button>
      {open && (
        <div className="absolute left-0 top-full z-50 mt-1.5 min-w-[200px] rounded-xl border border-slate-200 bg-white p-1.5 shadow-xl">
          {items.map((n) => (
            <NavLink
              key={n.to}
              to={n.to}
              ...
              className={({ isActive }) =>
                `flex items-center gap-2 rounded-lg px-3 py-2 text-sm font-medium ${
                  isActive ? 'bg-blood-light text-blood' : 'text-slate-600 hover:bg-slate-100 hover:text-brand-navy'
                }`
              }
            >
...
export default function AppLayout() {
  ...
  const navVisible = nav.slice(0, NAV_VISIBLE_COUNT);
  const navOverflow = nav.slice(NAV_VISIBLE_COUNT);
